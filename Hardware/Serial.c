...
{
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1,ENABLE);     //开启USART1和GPIOA时钟
...
void Serial_SendNumber(uint32_t Number, uint8_t Length)       //发送一个无符号数
{
    uint8_t i;
	for(i=0; i<Length; i++)
	{
...
void Serial_SendSignedNumber(int32_t Number, uint8_t Length)  //发送一个有符号数
 {
     uint8_t i;
	 uint32_t Number1;
	 if(Number>=0)
